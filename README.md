...
* Автоматическое распознование на снимках объектов трёх видов: животные (**animal**), люди (**person**), транспорт (**vehicle**).
* Рекурсивная обработка всех изображений во всех директориях входного каталога.
* Создание файла отчёта, с указанием для каждого файла-снимка:
  * отметки вида распознования (1 - animal, 2 - person, 3 - vehicle);
  * ранга. Ранжирование всех распознований для каждого снимка по степени уверенности (0, 1, 2, ...);
  * доверительного интервала (confidence interval), число меньше единицы и больше нуля (96.6 % — это точность работы обученных волонтеров в проекте Snapshot Serengeti);
  * четырёх координат, для выделения объекта на снимке.
...
Данный проект призван продемонстрировать работоспообность модели (предобученной нейронной сети **MegaDetctor**).
...
* Папку с тестовыми изображениями ```/megadetector_test_images```, всего 29 штук, из них:
...
](https://lila.science/datasets/snapshot-safari-2024-expansion/), один из датасетов для обучения модели.
    ...
    * * всё, кроме двух снимков, изображения из Википедии: динозавров, дамы в шляпе, танка, экскаватора, светофора, коня, единорога;
      ...
      * один снимок случайный из интернета ("осёл и кот.jpg"), для проверки распознания вымышленных существ из мультфильма.
